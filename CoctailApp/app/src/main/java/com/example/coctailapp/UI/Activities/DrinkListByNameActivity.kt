...
class DrinkListByNameActivity : AppCompatActivity() {
    ...
    fun getAllDrinksByName() {
        var s = RetrofitInitializer().serviceDrinks()
        var name = intent.getStringExtra("drink")
        var call = s.getByName(name)

        call.enqueue(object : retrofit2.Callback<DrinkList> {
            override fun onResponse(call: Call<DrinkList>?, response: Response<DrinkList>?) {
                response?.let {
                    if (it.code() == 200) {
                        list.adapter = DrinkListByNameAdapter(this@DrinkListByNameActivity, it.body().drinks)
                    }
                }
            }
